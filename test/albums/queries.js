const { query } = require('../server.spec'),
  ...
  { albumMock, albumsPhotosListMock, albumsFilteredListMock, albumsListMock } = require('./mocking'),
...
describe('albums', () => {
  describe('queries', () => {
    ...
    describe('Requests with missing or non-existent parameters', () => {
      it('request album without id', () => query(album()).then(res => testErrorResponse(res)));

      it('request albums without offset', () =>
        query(albums(null, 3, 'id', 'albumTitle')).then(res => testErrorResponse(res)));

      it('request albums without limit', () =>
        query(albums(0, null, 'id', 'albumTitle')).then(res => testErrorResponse(res)));

      it('request albums without orderBy', () =>
        query(albums(0, 3, null, 'albumTitle')).then(res => testErrorResponse(res)));
      ...
      it('request non-existent album', () => query(album(-10)).then(res => testErrorResponse(res)));
    });
  });
});
